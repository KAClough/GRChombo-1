    ...
    for (int icomp : extrapolating_comps)
    {
        ...
        if (a_side == Side::Hi)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                FOR1(idir) 
                {
                    if (iv_tmp[idir] > m_domain_box.bigEnd(dir))
                    {iv_tmp[idir] = m_domain_box.bigEnd(dir);}
                    else if (iv_tmp[idir] < m_domain_box.smallEnd(dir))
                    {iv_tmp[idir] = m_domain_box.smallEnd(dir);}
                }
